...
func TestScraper(t *testing.T) {
	stubMock := httptest.NewServer(http.HandlerFunc(func(rw http.ResponseWriter, req *http.Request) {
...
`))
	...
	sc := newStubScraper(zap.NewNop(), &Config{
		HTTPClientSettings: confighttp.HTTPClientSettings{
			Endpoint: stubMock.URL + "/server-status?auto",
		},
	})
	...
	for i := 0; i < ms.Len(); i++ {
		...
		case metadata.M.StubWorkers.Name():
			...
			require.Equal(t, map[string]int{
				"stub.workers state:busy": 13,
				"stub.workers state:idle": 227,
			}, workerMetrics)
		...
		case metadata.M.StubScoreboard.Name():
			...
			require.Equal(t, map[string]int{
				"stub.scoreboard state:open":         150,
				"stub.scoreboard state:waiting":      217,
				"stub.scoreboard state:starting":     1,
				"stub.scoreboard state:reading":      4,
				"stub.scoreboard state:sending":      12,
				"stub.scoreboard state:keepalive":    2,
				"stub.scoreboard state:dnslookup":    2,
				"stub.scoreboard state:closing":      4,
				"stub.scoreboard state:logging":      1,
				"stub.scoreboard state:finishing":    3,
				"stub.scoreboard state:idle_cleanup": 4,
			}, scoreboardMetrics)
