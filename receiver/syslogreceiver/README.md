...
The syslog receiver supports two processing modes:

### Raw Mode (`mode: raw`)

- **Use Case**: When you need to preserve the original syslog message format
- **Behavior**: Messages are passed through without parsing
- **Output**: Raw message in the log body
- **Benefits**: No data loss, works with any syslog format
- **Similar to**: TCP receiver behavior

### Parsed Mode (`mode: parsed`) - Default

- **Use Case**: When you need structured syslog data
- **Behavior**: Messages are parsed according to RFC standards
- **Output**: Structured fields in attributes and body
- **Benefits**: Rich metadata extraction, standardized format
- **Similar to**: Original syslog receiver behavior

## Configuration

| Field                               | Default      | Description                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| ----------------------------------- | ------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `mode`                              | `parsed`     | Processing mode for syslog messages. Options are `raw` and `parsed`. When `raw`, messages are passed through without parsing (like TCP receiver). When `parsed`, messages are parsed according to RFC standards.                                                                                                                                                                                                                                                 |
...
| `protocol`                          | required     | The protocol to parse the syslog messages as. Options are `rfc3164` and `rfc5424` (only used when mode is `parsed`)                                                                                                                                                                                                                                                                                                                                              |
...
### Parsed Mode Configuration (Default)
