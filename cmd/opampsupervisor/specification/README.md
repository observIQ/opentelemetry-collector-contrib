...
agent:
  ...
  # Optional configuration specifying how collector upgrades are formatted and verified
  package:
    # Name of the collector binary as it appears in the archive the supervisor downloads
    agent_binary: "otelcol-contrib"
    # Format of the archive downloaded by the supervisor
    archive: "tar.gz"
    # Configuration of the verification method used to verify the binary
    verifier:
      # Specifies which verification type should be used
      type: "cosign"
      # Read the "Cosign Overview" section below for more details
      cosign:
        # The Github repository that generated & signed the collector binary in a workflow
        github_workflow_repository: "open-telemetry/opentelemetry-collector-releases"
        # A set of identities to verify the signature matches against.
        # The `issuer` is the entity that issued an identity token and `subject` is the recipient of the token.
        identities:
          - issuer: "https://token.actions.githubusercontent.com"
            subject_regex: `^https://github.com/open-telemetry/opentelemetry-collector-releases/.github/workflows/base-release.yaml@refs/tags/[^/]*$`
